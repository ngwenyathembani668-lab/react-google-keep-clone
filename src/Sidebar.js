...
function Sidebar() {
  ...
  const renderSidebar = (id, isMobile = false) => (
    <aside
      id={id}
      className="sidebar"
      style={{ width: expanded ? "350px" : "90px" }}
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
    >
      {sidebarItems.map((item, index) => {
        const itemClass = isMobile && index === 0 ? "side-BarBtn" : "sideBarBtn";
        return (
          <div key={item.key} className={`${itemClass}${index === 0 ? " active" : ""}`}>
            <div className="svgBox">{item.icon}</div>
            <div className="sideText">
              <p className={`${item.labelClass} sidePTags`}>
                {expanded ? item.label : ""}
              </p>
            </div>
          </div>
